...
class TestMultiDict(unittest.TestCase):
    ...
    def test_two_members_no_overlap(self):
        first = {"a": 1, "b": 2, "c": 3}
        second = {"d": 1, "e": 2, "f": 3}
        md = dvt.utils.MultiDict([first, second])
        assert len(md) == 6
        for key in "abcdef":
            assert key in md
        assert md["a"] == 1
