...
class VerticaExtractor(BaseExtractor):
    ...
    def _get_connection(self, config: ExtractionConfig = None) -> Any:
        """Get or create a Vertica database connection."""
        ...
        try:
            ...
            raise ImportError(
                "vertica-python is required for Vertica extraction. Install with: pip install vertica-python"
            )

        from dvt.federation.auth.vertica import VerticaAuthHandler

        handler = VerticaAuthHandler()
        kwargs = handler.get_native_connection_kwargs(conn_config)
        self._lazy_connection = vertica_python.connect(**kwargs)
        return self._lazy_connection
