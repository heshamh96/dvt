...
dbt_function_full_names = set(["dvt.ref", "dvt.source", "dvt.config", "dvt.config.get"])
...
class PythonValidationVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef) -> None:
        if node.name == "model":
            ...
            if node.args.args and not node.args.args[0].arg == "dvt":
                self.dbt_errors.append("'dvt' not provided for model as the first argument")
            if len(node.args.args) != 2:
                self.dbt_errors.append(
                    "model function should have two args, `dvt` and a session to current warehouse"
                )
    ...
    def check_error(self, node):
        if self.num_model_def != 1:
            raise ParsingError(
                f"dvt allows exactly one model defined per python file, found {self.num_model_def}",
                node=node,
            )
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        for obj in node.args + [kwarg.value for kwarg in node.keywords]:
            if isinstance(obj, ast.Call):
                self.visit_Call(obj)
            # support dvt.ref in list args, kwargs
            elif isinstance(obj, ast.List) or isinstance(obj, ast.Tuple):
                for el in obj.elts:
                    if isinstance(el, ast.Call):
                        self.visit_Call(el)
            # support dvt.ref in dict args, kwargs
        ...
        if tree.body:
            ...
            for func, args, kwargs in dbt_parser.dbt_function_calls:
                if func == "get":
                    ...
                    if num_args == 0:
                        raise ParsingError(
                            "dvt.config.get() requires at least one argument",
                            node=node,
                        )
                    if num_args > 2:
                        raise ParsingError(
                            f"dvt.config.get() takes at most 2 arguments ({num_args} given)",
                            node=node,
                        )
                    key = args[0]
