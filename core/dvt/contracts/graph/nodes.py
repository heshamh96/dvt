...
from dvt.adapters.base import ConstraintSupport
from dvt.adapters.factory import get_adapter_constraint_support
...
class ModelNode(ModelResource, CompiledNode):
    ...
    def _normalize_data_type_for_comparison(data_type: Optional[str]) -> Optional[str]:
        ...
        Per dvt documentation, changes to size/precision/scale should not be
...
class SeedNode(SeedResource, ParsedNode):  # No SQLDefaults!
    ...
    def _disallow_implicit_dependencies(self):
        ...
        hook_list = "\n".join(hooks)
        message = f"""
Seeds cannot depend on other nodes. dvt detected a seed with a pre- or post-hook
