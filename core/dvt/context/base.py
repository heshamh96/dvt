...
class BaseContext(metaclass=ContextMeta):
    ...
    def dbt_version(self) -> str:
        """The `dbt_version` variable returns the installed version of dvt that
        ...
        > macros/get_version.sql
            ...
            {% macro get_version() %}
              {% set msg = "The installed version of dvt is: " ~ dbt_version %}
              {% do log(msg, info=true) %}
            {% endmacro %}

        Example output:

            $ dvt run-operation get_version
            The installed version of dvt is 0.16.0
        """
        return dbt_version
    ...
    def invocation_id(self) -> Optional[str]:
        """invocation_id outputs a UUID generated for this dvt run (useful for
        auditing)
        """
    ...
    def flags(self) -> Any:
        ...
        This supports all flags defined in flags submodule (core/dvt/flags.py)
        """
    ...
    def local_md5(value: str) -> str:
        ...
        It's called "local_md5" to emphasize that it runs locally in dvt (in jinja context) and not an MD5 SQL command.
