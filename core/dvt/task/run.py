...
class NonDefaultPushdownRunner(CompileRunner):
    ...
    When a model targets a non-default adapter (e.g., databricks when the
    default profile is postgres), the standard ModelRunner cannot be used
    because it's bound to the default adapter via the global FACTORY singleton.

    This runner uses AdapterManager to get the correct target adapter and
    executes the compiled SQL directly via CREATE TABLE AS / CREATE VIEW AS.
    ...
    def _execute_table(
        ...
        For full_refresh: DROP + CREATE TABLE AS SELECT
        Otherwise: DROP + CREATE TABLE AS SELECT (same for first run)
        """
    ...
    def _execute_incremental(
        ...
        For incremental without --full-refresh, the compiled SQL already
        contains the incremental logic (is_incremental() was true during
        Jinja compilation). We execute it as INSERT INTO.
        """
        # The compiled SQL for incremental models may already contain
        # a MERGE or INSERT statement depending on the strategy.
        # If the model SQL is a SELECT, wrap it in INSERT INTO.
...
class RunTask(CompileTask):
    ...
    def get_runner(self, node) -> BaseRunner:
        ...
        DVT override to support FederationModelRunner and
        NonDefaultPushdownRunner.

        Routing logic:
        - SPARK_FEDERATION → FederationModelRunner (Spark + JDBC)
        - ADAPTER_PUSHDOWN + non-default target → NonDefaultPushdownRunner
          (direct SQL on target adapter via AdapterManager)
        ...
        if node.unique_id in self._resolved_executions:
            ...
            # Pushdown on a non-default target: the default adapter (from FACTORY)
            # cannot execute SQL for a different target. Use AdapterManager instead.
            if (
        ...
        # Default behavior: same-target pushdown via standard ModelRunner
