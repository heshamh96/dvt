...
from dvt.adapters.contracts.connection import QueryComment
...
This version of dvt is not supported with the '{package}' package.
  Installed version of dvt: {installed}
  Required version of dvt for '{package}': {version_spec}
Check for a different version of the '{package}' package, or run dvt again with \
--no-version-check
"""
...
package.
  Required versions of dvt for '{package}': {version_spec}
Check for a different version of the '{package}' package, or run dvt again with \
--no-version-check
"""
...
def validate_version(dbt_version: List[VersionSpecifier], project_name: str):
    """Ensure this package works with the installed version of dvt."""
...
def _get_required_version(
    ...
    required = project_dict.get("require-dvt-version")
...
class PartialProject(RenderComponents):
    ...
    verify_version: bool = field(
        metadata=dict(description=("If True, verify the dvt version matches the required version"))
    )
...
class Project:
        ...
        result = deepcopy(
            {
                ...
                "require-dvt-version": [v.to_version_string() for v in self.dbt_version],
                "restrict-access": self.restrict_access,
                "dvt-cloud": self.dbt_cloud,
                "flags": self.flags,
            }
        )
    ...
    def get_default_selector_name(self) -> Union[str, None]:
        """This function fetch the default selector to use on `dvt run` (if any)
